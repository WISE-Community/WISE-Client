...
class NodeProgressViewController {
  ...
  getNumberOfStudentsOnNode(nodeId) {
    ...
    var count = this.classroomStatusService.getWorkgroupIdsOnNode(nodeId, periodId).length;
    return count;
  }
  ...
  getNodeCompletion(nodeId) {
    var currentPeriod = this.getCurrentPeriod();
    var periodId = currentPeriod.periodId;
    var completionPercentage = this.classroomStatusService.getNodeCompletion(nodeId, periodId)
      .completionPct;
  ...
  showRubric($event) {
    ...
    let windowString = `<link rel='stylesheet' href='/assets/wise5/themes/default/style/monitor.css'>
...
const NodeProgressView = {
  ...
  templateUrl:
    '/assets/wise5/classroomMonitor/classroomMonitorComponents/nodeProgress/nodeProgressView/nodeProgressView.html'
};

...
describe('ChatbotComponent', () => {
  ...
  describe('deleteChat', () => {
    it('should delete a chat after confirmation', async () => {
      const chat1 = createMockChat('1', 'Chat 1');
      const chat2 = createMockChat('2', 'Chat 2');
      const chats = [chat1, chat2];

      chatbotService.getChats.and.returnValue(of(chats));
      chatbotService.deleteChat.and.returnValue(Promise.resolve());
      fixture.detectChanges();
      ...
      await component['deleteChat'](chat1, event);

      expect(chatbotService.deleteChat).toHaveBeenCalledWith(runId, workgroupId, '1');
      expect(component['chats'].length).toBe(1);
      expect(component['chats'][0]).toEqual(chat2);
    });
    ...
    it('should not delete chat if user cancels confirmation', async () => {
      ...
      const event = new Event('click');
      await component['deleteChat'](chat, event);

      expect(chatbotService.deleteChat).not.toHaveBeenCalled();
      expect(component['chats'].length).toBe(1);
    });

    it('should switch to another chat when deleting current chat', async () => {
      const chat1 = createMockChat('1', 'Chat 1');
      const chat2 = createMockChat('2', 'Chat 2');
      const chats = [chat1, chat2];
    ...
    it('should create new chat when deleting the last chat', async () => {
      const chat = createMockChat('1', 'Chat 1');
      const newChat = createMockChat('2', 'New chat 1');
  ...
  describe('editChatTitle', () => {
    it('should update chat title', () => {
      const chat = createMockChat('1', 'Old Title');
      chatbotService.getChats.and.returnValue(of([chat]));
      chatbotService.updateChat.and.returnValue(Promise.resolve(chat));
      fixture.detectChanges();
      ...
      const event = new Event('click');
      component['editChatTitle'](chat, event);

      expect(chat.title).toBe('New Title');
      expect(chatbotService.updateChat).toHaveBeenCalledWith(runId, workgroupId, chat);
    });

    it('should not update chat title if user cancels', () => {
      const chat = createMockChat('1', 'Original Title');
      chatbotService.getChats.and.returnValue(of([chat]));
      fixture.detectChanges();
      ...
      expect(chatbotService.updateChat).not.toHaveBeenCalled();
    });

    it('should not update chat title if new title is empty', () => {
      const chat = createMockChat('1', 'Original Title');
      chatbotService.getChats.and.returnValue(of([chat]));
      fixture.detectChanges();
    ...
    it('should not update chat title if new title is same as old title', () => {
      const chat = createMockChat('1', 'Same Title');
      chatbotService.getChats.and.returnValue(of([chat]));
      fixture.detectChanges();

      component['chats'] = [chat];
      spyOn(window, 'prompt').and.returnValue('Same Title');
